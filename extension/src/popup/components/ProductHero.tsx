...
export const ProductHero: React.FC<ProductHeroProps> = ({ product }) => {
  ...
  return (
    <div className="flex gap-4 items-start p-3 bg-[#14151a] border border-[#22242b] rounded-lg">
      ...
      <div className="flex-1 min-w-0">
        <span className="text-[10px] font-mono uppercase tracking-widest text-[#c85a32] block font-medium">
          {bestOffer?.store_name || 'Loja Parceira'} • {product.category_name || 'Nicho Masculino'}
        </span>
        <h3 className="text-sm font-semibold text-white tracking-tight mt-0.5 truncate">
          {product.name}
        </h3>
        <p className="text-xs text-[#71717a] font-mono">100ml / Edição Limitada</p>
        ...
        <div className="mt-2 flex items-baseline gap-2">
          ...
          <span className="text-[10px] text-[#c85a32] font-mono ml-auto font-medium">
            ↓ 12% 30d
          </span>
        </div>
      </div>

...
Migrar a infraestrutura (por exemplo Google Cloud, Cloud SQL, Cloud Run) **não é autorização** para alterar a arquitetura atual. Obrigatório:

```text
PROPOSE DECISION
```

Primeiro decide-se a arquitetura de infraestrutura. **Depois** autoriza-se a implementação.
...
Se houver conflito entre `PGRST_DB_URI` / role de conexão e o modelo de autenticação/RLS:

```text
STOP AND REPORT
```

Não editar `.env`, SQL ou PostgREST apenas para “fazer funcionar”.
...
│   │   │
│   │   ├── jobs/
│   │   │   ├── ingest-prices.job.ts
│   │   │   ├── calculate-deals.job.ts
│   │   │   └── process-alerts.job.ts
│   │   │
│   │   ├── stores/
│   │   ├── services/
│   │   ├── database/
...
* migração de hospedagem (incluindo Google Cloud, Cloud SQL, Cloud Run);
...
# 102. INSTRUÇÃO FINAL PARA O CURSOR — GUARDRAIL DE EXECUÇÃO

Ao receber qualquer tarefa deste projeto:

> **Primeiro entenda o sistema existente. Depois implemente somente o necessário.**

`PROJECT_CONTEXT.md` é **contexto**, não autorização irrestrita para modificar o projeto.

A implementação deve partir da combinação:
...
Se houver conflito entre contexto, código, schema ou regra:

```text
REPORT CONFLICT
```

Se houver mudança arquitetural ou de contrato (incluindo hospedagem):
...
Divergências **não podem ser corrigidas silenciosamente**.

Exemplos obrigatórios:

* Migração futura para Google Cloud / Cloud SQL / Cloud Run → `PROPOSE DECISION`. Não interpretar discussão de infra como autorização para alterar a arquitetura atual.
* Conflito entre `PGRST_DB_URI` e o modelo de autenticação/RLS (role `authenticator`) → `STOP AND REPORT`. Não editar `.env`, SQL ou PostgREST apenas para “fazer funcionar”.
